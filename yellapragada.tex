...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[RE,RO]{J S\&E \textbf{2}, 86--88 (2026)} % FOR EDITOR ONLY
\fancyhead[LO]{Coulanges \emph{et al}} % if many authors
...
\fancyhf{}
\fancyhead[C]{Journal of Science \& Engineering \textbf{2}, 86--88 (2026)} % FOR EDITOR ONLY
\fancyfoot[C]{\thepage}
...
\begin{document}
\setcounter{page}{86} % FOR EDITOR USE ONLY
...
\noindent DOI: \href{https://doi.org/10.64804/2t93cp73}{10.64808/2t93cp73}
\end{abstract}
...
Data analysis and the force-compression graph were done in Google Sheets (Mountain View, CA) \cite{starnes:2015:practice}. \texttt{R} with library \texttt{ggplot2} \cite{R,ggplot2,dplyr}, was used to create a bar chart comparing the mean maximum jump height for happy and sad poppers and to perform additional statistical analyses and graphing. Data and code are provided in \url{https://github.com/devangel77b/427syellapragada-lab3}. 

The experimental setup is illustrated in \cref{fig:setup}. The setup for measuring $EPE$ is illustrated in \cref{fig:setup_epe}.
\begin{figure}
\begin{center}
\includesvg{figures/setupphoto.svg}
\end{center}
...
\section{Results} 
The measured mass, maximum jump height, and velocity for the happy and sad popper groups are summarized in \cref{tab:happy} and \cref{tab:sad}, respectively.
\begin{table}
\caption{Mass $m$, height $h$, and velocity data for happy poppers.}
\label{tab:happy} 
\begin{ruledtabular}
\begin{tabular}{cS[table-format=1.4]S[table-format=1.2]S[table-format=3.1]} 
trial & {$m$, \unit{\kilo\gram}} & {$h$, \unit{\meter}} & {$v_{\text{launch}}$, \unit{\meter\per\second}}\\
\colrule \\
1 & 0.0058 & 2.12 & 6.26 \\
2 & 0.0057 & 1.82 & 6.10 \\
3 & 0.0056 & 1.47 & 5.16 \\
...
5 & 0.0056 & 1.54 & 5.39 \\
\colrule 
mean & 0.0057 & 1.72 & 5.74 \\
sd & 0.00008 & 0.26 & 0.46 \\
\end{tabular} 
\end{ruledtabular}
\end{table}
\begin{table}
\caption{Mass $m$, height $h$, and velocity data for sad poppers.}
\label{tab:sad} 
\begin{ruledtabular}
...
5 & 0.0055 & 1.58 & 5.62 \\
\colrule 
mean & 0.0055 & 1.55 & 5.46 \\
sd & 0.0002 & 0.16 & 0.34 \\
\end{tabular} 
\end{ruledtabular}
\end{table}

The initial kinetic energy ($KE$) and maximum gravitational potential energy ($GPE$) for the happy and sad popper groups are summarized in \cref{tab:happy2} and \cref{tab:sad2}, respectively.
\begin{table}
\caption{Energy data for happy poppers, based on \cref{tab:happy}.}
\label{tab:happy2} 
\begin{ruledtabular}
\begin{tabular}{cS[table-format=1.4]S[table-format=1.2]S[table-format=3.1]} 
trial & {$KE$, \unit{\milli\joule}} & {$GPE$, \unit{\milli\joule}} & {KE - GPE, \unit{\milli\joule}} \\
\colrule \\
1 & 113.6 & 120.6 & 7.0 \\
2 & 105.9 & 101.8 & 4.1 \\
3 & 74.4 & 80.8 & 6.3 \\
...
\end{table}
\begin{table}
\caption{Energy data for sad poppers, based on \cref{tab:sad}.}
\label{tab:sad2} 
\begin{ruledtabular}
\begin{tabular}{cS[table-format=1.4]S[table-format=1.2]S[table-format=2.1]} 
trial & {$KE$, \unit{\milli\joule}} & {$GPE$, \unit{\milli\joule}} & {KE - GPE, \unit{\milli\joule}} \\
...
Force-compression data are shown in \cref{fig:force_graph}. Total ($EPE$) was calculated by integrating the area under the curve in three segments, giving \qty{294.0}{\milli\joule}.
\begin{figure}
\begin{center}
\includesvg{fig3.svg}
\end{center}
\caption{Force versus compression graph used to calculate total $EPE$ via area under the curve. The total $EPE$ is \qty{294}{\milli\joule}.}
...
\begin{table}
\caption{Two-sample $t$-test results for mass $m$ and height $h$; data from \cref{tab:happy,tab:sad}.}
\label{tab:stats} 
\begin{ruledtabular}
\begin{tabular}{lSSSS} 
...
\begin{table} 
\caption{Paired $t$-test results for $KE$ at launch and $GPE$ at peak; data from \cref{tab:happy2,tab:sad2}.}
...
\section{Discussion}
The results of this experiment indicate that popper design does not have a statistically significant effect on maximum jump height. Although the mean height of the happy poppers (\qty{1.72}{\meter}) was slightly greater than that of the sad poppers (\qty{1.55}{\meter}), the distributions overlap substantially, as indicated by \cref{fig:2}. \cref{tab:stats} shows that $p=0.24$ for height, which is greater than $\alpha=0.05$. Therefore, we fail to reject the null hypothesis. Additionally, no significant difference was found in mass ($p = 0.12$), indicating that the height difference is attributable to other factors rather than mass being a confounding variable. 
...
A possible source of experimental error is the force at which each popper is launched. Despite the fixed location, the launch was performed by an individual rather than a repeatable machine; thus, the angle, speed, and pressure applied during compression may have varied, causing variability in the data. 

Additionally, error in human reaction and observation during video analysis may have occurred \cite{faux:2019:manual}. Measurements taken from a meter stick in videos can be misread and are also subject to lens distortion, leading to inconsistencies \cite{chazot:2025:using,tracker}. 

Finally, material wear is a likely cause of error. As the poppers had been used previously, internal deformations in the rubber may have altered the elastic properties over time, leading to inconsistent energy results \cite{simeonov:2016:coefficient,sendrowicz2022stored}.
